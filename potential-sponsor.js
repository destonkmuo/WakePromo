async function loadDictionary() {
  ...
  class PotentialSponsor {
	...
	incSponsorFrequency(set, weight, sponsorSet) {
		set.forEach((element) => {
		  if (sponsorSet[element] == null)
			sponsorSet[element] = 0;
	...
	cleanSponsorFrequency() {
	  ...
	  //Generate Context
	  const videoAttributes = [this.title, this.channelTitle, this.tags, this.channelTitle];
		...
		this.synonyms.forEach((synonym) => {
		  if (sentence.includes(synonym)) {
			words.forEach((word) => {
				if (word.length > 3) {
					for (const potentialSponsor in this.PotentialSponsors) {
						if (similarity(word, potentialSponsor) > 0.7 || word.includes(potentialSponsor) || potentialSponsor.includes(word))
						  result.add(potentialSponsor);
					  }
				} 
			});
