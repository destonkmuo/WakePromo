...
class PotentialSponsor {
	...
	cleanClusters() {
		for (const sponsor in this.sponsorClusters) {
			if (
				this.sponsorClusters[sponsor].startTime == this.sponsorClusters[sponsor].endTime
			) {
	...
	incSponsorFrequency(set, weight) {
		set.forEach((element) => {
			if (this.PotentialSponsors[element] == null)
				this.PotentialSponsors[element] = 0;
	...
	cleanSponsorFrequency() {
		...
		context.forEach((context) => {
			for (const sponsor in this.PotentialSponsors) {
				...
				if (
					similarity(sponsor, context) > 0.7 ||
	...
	async spellCheck() {
		...
		description.forEach((word) => {
			...
			const trimmedWord =
				word.match(descRegExp) != null ? word.match(descRegExp)[0].toLowerCase() : '';
		...
		description.forEach((word) => {
			for (const company in response.companies) {
				...
				if (
					...
					similarity(response.companies[company].toLowerCase(), trimmedWord) > 0.7
				) {
					for (const potentialSponsor in this.PotentialSponsors) {
